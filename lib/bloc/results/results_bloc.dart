import 'package:quiz_bloc/bloc/results/results_event.dart';
import 'package:quiz_bloc/bloc/results/results_state.dart';
...
class ResultsBloc extends Bloc<ResultsEvent, ResultsState> {
  ...
  ResultsBloc()
      : super(
            Success(null, null, 'Choose category', 'Choose difficulty level')) {
    on<CategoryDataRequested>((event, emit) async {
      ...
      try {
        ...
        if (result) {
          final data =
              await repo.getLastTenScores(event.category, event.diffLevel);
          if (event.category == 'None') {
            emit(Success(const [], const [], 'Choose category',
